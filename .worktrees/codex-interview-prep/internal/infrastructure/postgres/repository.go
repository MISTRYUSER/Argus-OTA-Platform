...
// SaveBatchAndFile 在同一事务内保存 batch 与 file，避免跨表写入不一致。
func (r *PostgresBatchRepository) SaveBatchAndFile(ctx context.Context, batch *domain.Batch, file *domain.File) error {
	...
	batchQuery := `
          ...
          ON CONFLICT (id) DO UPDATE SET
              ...
              updated_at = EXCLUDED.updated_at
      `
	if _, err := tx.ExecContext(ctx, batchQuery,
		batch.ID, batch.VehicleID, batch.VIN, batch.Status.String(), batch.UploadTime,
		batch.TotalFiles, batch.ProcessedFiles, batch.ExpectedWorkerCount,
		batch.CompletedWorkerCount, batch.MinIOBucket, batch.MiniIOPrefix,
		batch.ErrorMessage, batch.CompletedAt, batch.CreatedAt, batch.UpdatedAt,
	); err != nil {
		return fmt.Errorf("failed to save batch in tx: %w", err)
	}
	...
	fileQuery := `
		...
		ON CONFLICT (id) DO UPDATE SET
			...
			updated_at = EXCLUDED.updated_at
	`
	if _, err := tx.ExecContext(ctx, fileQuery,
		...
		return fmt.Errorf("failed to save file in tx: %w", err)
	}
