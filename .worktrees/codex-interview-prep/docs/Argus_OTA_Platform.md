...
- **系统完整度**：85%
**说明（务必阅读）**：本文包含“已实现”与“路线图”两部分。  
**当前实现现状**以「实现状态快照」为准，其他章节如未标注则视为路线图或设计目标。
...
## ✅ 实现状态快照（Reality Check）

**当前已实现**  
- Query Service：`internal/application/query_service.go`（Singleflight + Redis Cache）  
- ReportRepo（真实实现）：`internal/infrastructure/postgres/report_repository.go`  
- AI Agent：Sequential Graph（DataLoader → RAG → LLM）  
- RAG 降级标记：`RAGUnavailable`

**当前未实现（路线图）**  
- Supervisor 动态路由  
- pgvector 混合检索 + Embedding 实接入（当前 VectorRetriever 为 mock）  
- Kafka Consumer 主流程接入 + 诊断结果回写  
- SSE 订阅端（发布端已有）  
- Python Worker（当前阶段可忽略）
...
## 0.9 RAG 搭建（第一性原理 + Eino 标准）

> **目标**：在本仓库内落地可执行的 RAG 链路，并严格遵守 Eino 标准接口。  
> **参考背景知识**：`docs/background/*`、`docs/background/code_review_feedback.md`

### 0.9.1 第一性原理（为什么必须 RAG）

1. **诊断准确性不是“聪明”，而是“可验证知识”**  
   车辆故障是强领域问题，通用模型记忆不足 → 必须用历史案例/知识库补全。  
2. **召回可控比“相似度高”更重要**  
   先结构化硬过滤（车型/错误码/版本），再语义排序，避免跨车型误诊。  
3. **降级必须显式**  
   RAG 失败时必须告诉 LLM “知识库不可用”，否则会产生幻觉。

### 0.9.2 数据流（最小闭环）

```
AggregatedData → Embedding → SQL 硬过滤 → pgvector 相似度排序 → TopK → LLM
```

### 0.9.3 Eino 标准（必须遵守）

- **LLM 必须使用 `model.ChatModel`**（禁止裸 HTTP）  
- **Embedding 必须使用 `model.EmbeddingModel`**（禁止自定义 HTTP 包装）  
- **工具必须用 `tool.Utils.InferTool`**（统一 ToolInfo 生成）  
- **Graph 用 `compose.Chain`/`compose.Graph`**  
- **RAG 失败必须设置 `RAGUnavailable=true` 并继续流程**  
- **JSON Mode / Token 统计 / Tracing** 必须开启

> 标准来源：  
> `docs/background/1.Eino框架单Agent开发文档.md`  
> `docs/background/2.Eino 框架 ai 工具开发文档.md`  
> `docs/background/code_review_feedback.md`

### 0.9.4 落地步骤（结合本仓库）

**Step 1: pgvector 环境与表结构**  
- 启用扩展：`CREATE EXTENSION IF NOT EXISTS vector;`  
- 使用 `ai_diagnoses.embedding` 或知识库表  
- **注意**：向量是否归一化决定使用 `<=>` 或 `<#>`（见 `code_review_feedback.md`）

**Step 2: EmbeddingModel 接入（Eino 标准）**  
- 文件：`workers/ai-agent/internal/infrastructure/llm/glm4_embedding.go`  
- 使用 Eino 的 `model.EmbeddingModel`（如 `eino-ext/components/embedding/openai`）

**Step 3: pgvector Retriever 实现**  
...
**Step 4: 接入 RAGNode**  
- `workers/ai-agent/internal/application/nodes/rag_node.go`  
- 失败必须设置 `RAGUnavailable=true`

**Step 5: 进入诊断流程**  
- `workers/ai-agent/internal/application/diagnosis_graph.go`  
- 当前是 Sequential Graph：DataLoader → RAG → LLM

### 0.9.5 完成清单（你要做的五件事）

1. **pgvector 检索器 + Embedding 接入**：`workers/ai-agent/TASKS.md`
2. **Kafka Consumer 接入主流程 + 诊断结果回写**：`workers/ai-agent/internal/infrastructure/kafka/consumer.go`
3. **Supervisor/动态路由**：参照 `docs/eino_design.md` 的 v2.0 设计
4. **Query Service 真实 ReportRepo**：已完成（`internal/infrastructure/postgres/report_repository.go`）
5. **SSE 订阅端**：发布端已存在 `event_publisher.go`，需补 HTTP SSE
...
**实现代码**：`internal/application/query_service.go`

### 2.4 AI 智能诊断（当前为 Sequential Graph）

**状态**：已实现 v1.0（Sequential Graph），Multi-Agent/ Supervisor 为路线图

#### **架构设计**

```
AI Diagnose Service（当前实现：Sequential Graph）
├── Domain Layer
│   ├── DiagnosisContext / Diagnosis / Repository 接口
│
├── Application Layer
│   ├── DiagnosisGraph（DataLoader → RAG → LLM）
│   └── MultiAgentService（封装诊断入口）
│
├── Infrastructure Layer
│   ├── PostgresDiagnosisRepository
│   ├── LLM Provider（Eino Model 接口）
│   ├── VectorRetriever（当前为 mock，待 pgvector）
│   └── Redis EventPublisher（进度发布端）
│
└── Interfaces Layer
    └── Kafka Consumer（已存在，但未接入主流程）
```

#### **当前 RAG Pipeline（已实现）**

```
DataLoader → RAG → LLM → Output
```

#### **路线图（未实现）**

- Supervisor / 动态路由  
- pgvector 混合检索 + Embedding 实接入  
- Kafka Consumer 主流程接入 + 诊断结果回写  

> 详细落地步骤见「0.9 RAG 搭建」章节。
...
├── Domain Layer
│   ├── Report 聚合根（结构化字段）
│   │   ├── ID, BatchID, VehicleID, VIN
│   │   ├── Status, TotalFiles, ProcessedFiles
│   │   ├── CPUStats, RAMStats, DiagnosisResult, DiagnosisID
│   └── ReportRepository 接口
│       ├── FindByID(ctx, id) (*Report, error)
│       ├── FindByBatchID(ctx, batchID) (*Report, error)
│       └── Save(ctx, report) error
│
├── Application Layer
│   └── QueryService（用例编排）
│       ├── GetReport(ctx, batchID) (*Report, error)  ← Singleflight 核心
│       └── GetProgress(ctx, batchID) (map[string]interface{}, error)
│
├── Infrastructure Layer
...
> 实现细节：`reports.report_data` 存储 `domain.Report` 的 JSONB；`report_type` 固定为 `batch_report`。
```
...
func (s *QueryService) GetReport(ctx context.Context, batchID uuid.UUID) (*domain.Report, error) {
    // 1. Singleflight 保护（防止缓存击穿）
    result, err, shared := s.sf.Do(batchID.String(), func() (interface{}, error) {
        // 1.1 缓存读取
        if cached, _ := s.cache.GET(ctx, fmt.Sprintf("report:%s", batchID)); cached != "" {
        ...
        _ = s.cache.SET(ctx, fmt.Sprintf("report:%s", batchID), string(data), 10*time.Minute)
...
## 8.5 Multi-Agent + DDD 融合架构设计 ⭐⭐⭐ NEW

### 8.5.1 设计目标

将 **Multi-Agent** 融入 **DDD 架构**，实现：
- **依赖倒置**: Application 层依赖 Domain 层接口，不依赖具体实现
- **单一职责**: 每个 Agent 只负责单一任务（分析/检索/诊断）
- **协作模式**: Supervisor 模式（中心化调度）+ WithDeterministicTransferTo（自动回调）
- **可测试性**: 可以 Mock LLMClient 和 VectorRetriever
...
│   │   ├── llm_client.go            # LLM Client 接口（抽象）
│   │   ├── vector_retriever.go      # 向量检索器接口（抽象）
│   │   └── repository.go            # Repository 接口（抽象）
│   │
│   ├── application/                 # 应用层（Multi-Agent 编排）
│   │   ├── multi_agent_service.go   # Multi-Agent 协作服务（核心）
│   │   ├── agents/                  # 单个 Agent 实现
│   │   │   ├── analysis_agent.go    # 数据分析 Agent（剪枝）
│   │   │   ├── retrieval_agent.go   # RAG 检索 Agent
│   │   │   └── diagnosis_agent.go   # 诊断 Agent（LLM）
│   │   └── supervisor_agent.go      # Supervisor 调度 Agent
│   │
│   ├── infrastructure/              # 基础设施层（技术实现）
│   │   ├── llm/eino_client.go       # Eino Client 实现
│   │   ├── rag/vector_retriever.go  # pgvector 检索器
│   │   ├── postgres/diagnosis_repo.go
│   │   └── kafka/consumer.go        # Kafka Consumer
│   │
│   └── interfaces/http/
│       └── handler.go               # 健康检查端点
...
### 8.5.3 Multi-Agent 协作模式: Supervisor

**Agent 职责划分**:

| Agent | 职责 | 输入 | 输出 |
|-------|------|------|------|
| **AnalysisAgent** | 数据分析、剪枝 | 聚合数据（原始） | Summary（Top-K 异常码） |
| **RetrievalAgent** | RAG 检索 | Top-K 异常码 | 相似案例列表 |
| **DiagnosisAgent** | LLM 诊断 | Summary + 相似案例 | 诊断结果 |
| **DiagnosisSupervisor** | 中心调度 | Batch ID | 最终诊断结果 |

**协作流程**:

```
用户请求（Kafka 消息: GatheringCompleted）
    ↓
DiagnosisSupervisor 接收任务
    ↓
转让给 AnalysisAgent（数据剪枝）
    ├─ ReAct 循环：调用 rank_errors 工具
    ├─ ReAct 循环：调用 mark_critical 工具
    └─ 返回 Summary → 自动回调 Supervisor
    ↓
转让给 RetrievalAgent（RAG 检索）
    ├─ ReAct 循环：调用 get_embedding 工具
    ├─ ReAct 循环：调用 vector_search 工具
    └─ 返回相似案例 → 自动回调 Supervisor
    ↓
转让给 DiagnosisAgent（LLM 诊断）
    ├─ ReAct 循环：构造 Prompt
    ├─ ReAct 循环：调用 LLM（GPT-4）
    └─ 返回诊断结果 → 自动回调 Supervisor
    ↓
Supervisor 输出最终结果 → 发布 DiagnosisCompleted 事件
```

**核心代码 - Supervisor 实现**:

```go
// internal/application/supervisor_agent.go
func NewDiagnosisSupervisor(
    analysisAgent adk.Agent,
    retrievalAgent adk.Agent,
    diagnosisAgent adk.Agent,
) adk.Agent {
    supervisorName := "DiagnosisSupervisor"

    // Supervisor 配置
    supervisorConfig := &adk.ChatModelAgentConfig{
        Name:        supervisorName,
        Description: "智能诊断调度中心",
        Instruction: `你是诊断调度专家，遵循以下流程：
1. 接收 Batch ID 和聚合数据
2. 转让给 AnalysisAgent 进行数据剪枝
3. AnalysisAgent 完成后，转让给 RetrievalAgent 检索相似案例
4. RetrievalAgent 完成后，转让给 DiagnosisAgent 生成诊断结果
5. DiagnosisAgent 完成后，输出最终诊断结果`,
        Model: chat.CreateOpenAiChatModel(ctx),
    }

    supervisor, _ := adk.NewChatModelAgent(ctx, supervisorConfig)

    // 注册子 Agent（关键：让 Supervisor 能找到子 Agent）
    registeredSupervisor, _ := adk.SetSubAgents(ctx, supervisor, []adk.Agent{
        analysisAgent,
        retrievalAgent,
        diagnosisAgent,
    })
...
```go
// internal/application/agents/analysis_agent.go
func NewAnalysisAgent(supervisorName string) adk.Agent {
    ctx := context.Background()

    // 创建基础 Agent
    baseAgent, _ := adk.NewChatModelAgent(ctx, &adk.ChatModelAgentConfig{
        Name:        "AnalysisAgent",
        Description: "分析聚合数据，提取 Top-K 异常码",
        Instruction: `你是数据分析专家：
1. 读取聚合数据（总日志数、错误码分布）
2. 提取 Top-10 异常码（按频率排序）
3. 标记严重错误（Critical 级别）
4. 输出结构化 Summary（JSON 格式）`,
        Model: chat.CreateOpenAiChatModel(ctx),
    })

    // 增强：完成后自动回调 Supervisor
...
```go
// internal/domain/llm_client.go
package domain

import "context"

// LLMClient - LLM 客户端接口（依赖倒置）
...
```go
// internal/domain/vector_retriever.go
package domain
...
### 8.5.5 Infrastructure 层（技术实现）

```go
// internal/infrastructure/llm/eino_client.go
package llm
...
// EinoClient - Eino LLM Client 实现（依赖 OpenAI）
type EinoClient struct {
    model model.ToolCallingChatModel
}

// 实现 Domain 层定义的 LLMClient 接口
func (c *EinoClient) Diagnose(ctx context.Context, prompt string) (*domain.DiagnosisResponse, error) {
    // 调用 Eino 模型
    // ...
}
```
...
```go
// internal/infrastructure/rag/vector_retriever.go
package rag
...
// VectorRetriever - pgvector 检索器实现
type VectorRetriever struct {
    db          *sql.DB
    llmClient   domain.LLMClient // 复用 LLM Client 生成 Embedding
}

// 实现 Domain 层定义的 VectorRetriever 接口
func (r *VectorRetriever) Retrieve(ctx context.Context, query string, topK int) ([]domain.SimilarCase, error) {
    // 1. 生成 Embedding
    embedding, err := r.llmClient.GetEmbedding(ctx, query)

    // 2. pgvector 相似度搜索
    rows, err := r.db.QueryContext(ctx, `
        SELECT d.id, d.result, d.vehicle_id,
...
func main() {
    ...
    // 1. 初始化基础设施
    db := initPostgreSQL()
    kafkaReader := initKafkaConsumer()

    // 2. 创建 Repository（Infrastructure）
    diagnosisRepo := postgres.NewDiagnosisRepository(db)

    // 3. 创建 LLM Client（Infrastructure，实现 Domain 接口）
    llmClient, _ := llm.NewEinoClient(ctx, os.Getenv("OPENAI_API_KEY"))

    // 4. 创建 Vector Retriever（Infrastructure，实现 Domain 接口）
    vectorRetriever := rag.NewVectorRetriever(db, llmClient)

    // 5. 创建 Agent（Application 层）
    analysisAgent := agents.NewAnalysisAgent("DiagnosisSupervisor")
    retrievalAgent := agents.NewRetrievalAgent("DiagnosisSupervisor", vectorRetriever)
    diagnosisAgent := agents.NewDiagnosisAgent("DiagnosisSupervisor", llmClient)

    // 6. 创建 Supervisor（Application 层）
    supervisorAgent := application.NewDiagnosisSupervisor(
        analysisAgent,
        retrievalAgent,
        diagnosisAgent,
    )

    // 7. 创建 Multi-Agent Service（Application 层）
    multiAgentService := application.NewMultiAgentService(
        supervisorAgent,
        diagnosisRepo,
        application.NewTokenTracker(100000),
    )

    // 8. 创建 Kafka Consumer
    consumer := kafka.NewConsumer(kafkaReader, multiAgentService)
...
**A**:
- **分层清晰**: Domain/Application/Infrastructure 职责明确
- **依赖倒置**: Application 依赖 Domain 接口，不依赖 Infrastructure
- **可测试性**: 可以 Mock LLMClient 和 VectorRetriever
- **可替换性**: 轻松从 OpenAI 切换到本地模型

**Q: 为什么选择 Supervisor 模式？**

**A**:
- **动态决策**: LLM 根据结果判断下一步（查库还是直接生成报告）
- **自动回调**: WithDeterministicTransferTo 实现子 Agent 完成后自动回调
- **灵活调整**: 比 Sequential 更灵活（诊断失败可重新检索）

**Q: ReAct 范式的作用？**

**A**:
- **传统 LLM**: 直接回答问题（无法调用工具）
- **ReAct LLM**: 思考 → 调用工具 → 观察结果 → 再思考 → 再行动
- **Eino 实现**: ChatModelAgent 自动实现 ReAct 循环

**Q: 如何控制 Token 成本？**

**A**:
- **Summary 剪枝**: 只保留 Top-K 异常码
- **Token 追踪**: 记录每次调用的 Token 使用
- **每日限额**: 设置每日 10 万 Token 上限
